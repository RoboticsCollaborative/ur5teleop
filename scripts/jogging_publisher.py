...
class Jogging_Publisher():
    ...
    def __init__(self, daq_device, rate = 100):
        ...
        #setup publisher
        self.jogging_pub = rospy.Publisher('jogging', Bool, queue_size=1)
        #rospy.init_node('daqnode')

    def sample_publish(self):
        input=self.dio_device.d_bit_in(self.port_to_read,1)
        self.jogging_pub.publish(Bool(data = not input))
...
if __name__ == '__main__':
    ...
    dp = Jogging_Publisher(daq_device, rate = 100)
    dp.run()
